      ...
      <v-card-text>
        ...
        <div :style="enableInput ? 'border: solid 2px #f0f0f0; border-radius: 4px' : ''">
          ...
          <div v-if="enableInput" style="padding: 5px">
            <v-select 
              ...
              variant="underlined" 
              v-model="$store.state.sendObj.type"
              item-title="name" 
              item-value="value" 
        ...
        <div>
          ...
          <div style="display: flex;">
            <div v-for="col in $store.state.sendObj.colors" :key="col" @click="selectColor(col)" class="color-balls"
        ...
        <!-- $store.state.sendObj {{ $store.state.sendObj }} -->
      </v-card-text>
    </v-card>
  </div>
</template>
